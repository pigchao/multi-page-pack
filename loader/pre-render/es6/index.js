...
class PuppeteerRenderer {
  ...
  async handleRequestInterception(page, baseURL) {
    ...
    page.on('request', req => {
      ...
      if(typeof this._rendererOptions.dealWithRequest === 'function'){
        req.continue(this._rendererOptions.dealWithRequest(req, baseURL))
      }else{
        req.continue();
      }
        ...
        (route, index) => limiter(
          async () => {
            ...
            const result = {
              ...
              outputPath: path.join(rootOptions.outputPath, route)
            };
